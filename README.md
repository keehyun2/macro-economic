...
- **대시보드** — 핵심 지표 8종 + 이번 달 승자·패자(11개 가계 페르소나 점수) + 거시 배경
  (경제심리지수, 경기종합지수, M2, 경상수지, 소비자·생산자물가) + 당일 원/달러
  매매기준율 칩(731Y001 일별)
- **영끌족** — 월 상환 시뮬레이터(원금 슬라이더, 변동/고정 토글, 만기일시(이자만)/
  원리금균등 상환방식 토글 + 만기 10~40년 선택), 금리 역사 백분위,
  신규취급 vs 잔액 기준 금리, 매매가 vs 전세가, 가계 주택관련대출 규모, 집값 전년비
- **빚투족** — 손익분기 수익률(신용대출 금리), 레버리지 스프레드(KOSPI 12M − 차입금리),
  차입비용의 시장판(CD·회사채·국고채와 은행 금리 비교)
- **예금족** — 실질예금금리(예금금리 − CPI·PPI), 이자 시뮬레이터(원금 슬라이더,
  세전/세후 15.4% 토글), 예금 상품별 금리(적금·MMDA), 예금금리 백분위,
  예금 vs 국고채(3년), 은행 수신잔액
- **승자·패자** — 현재 시장 상태, 시나리오(금리×물가×환율) × 페르소나 히트맵,
  과거 국면 리플레이(코로나 저금리기 · 긴축 인상기 · 최근 1년), 점수 모델 전문 공개
- **데이터** — 사용 통계표 29종(통계표/항목코드 포함)과 데이터 시점, 이용 안내
...
src/lib/
  ...
  personas.ts             페르소나 11종 정의 + 점수 모델
src/app/                  페이지 6종 + /api/refresh (크론용 재검증 엔드포인트)
```
...
- 대시보드의 당일 원/달러 칩은 `series.json`의 `chipOnly` 정의(731Y001 일별)로
  `ecos.ts`의 `fetchDailyLatest`가 최근 영업일 매매기준율만 가져온다. 월평균
  파이프라인·스냅샷에서는 제외되고(정규화 미러 규칙 대상 아님) 조회 실패 시 칩이 숨겨진다.
...
한국은행 경제통계시스템(ECOS) — 기준금리·대출/예금 금리(신규·잔액 기준)·시장금리(국고채·CD·
회사채)·환율·가계신용·M2·경기종합지수·국제수지·은행 수신잔액·경제심리지수·국제 주요국
주가지수(미국, 해외투자자 요인용), 그리고 ECOS에
수록된 통계청(소비자·생산자물가)·KB 주택가격 동향(매매·전세가격지수).
